...
class CrosswordCreator():
    ...
    def solve(self, interleaving=False):
        """
    ...
    def ac3(self, arcs=None):
        ...
        if arcs is None:
            arcs = [(var1, var2)
                    for var1 in self.domains for var2 in self.crossword.neighbors(var1)]
            ...
            for neighbor in self.crossword.neighbors(var):
                if neighbor not in assignment:
                    for neighbor_val in self.domains[neighbor]:
                        if val == neighbor_val or not self.overlap_satisfied(var, neighbor, val, neighbor_val):
                            vals_ruleout[val] += 1

        # Return list of variable's domain values in sorted order
        return sorted(self.domains[var], key=lambda val: vals_ruleout[val])
    ...
    def select_unassigned_variable(self, assignment):
        ...
        # Dictionary to store remaining values for each unassigned variable
        ...
        # Get minimum number of remaining values
        min_remaining = min(num_remaining_values.values())

        # Get all variables with the minimum remaining values
        candidate_vars = [
            var for var in num_remaining_values
            if num_remaining_values[var] == min_remaining
        ]

        # If only one candidate variable, return it
        if len(candidate_vars) == 1:
            return candidate_vars[0]

        # Otherwise, return variable with highest degree (most neighbors)
        degrees = {
    ...
    def backtrack(self, assignment):
        ...
        # Track number of calls to backtrack
        BACKTRACK_COUNTER += 1
        ...
        # Iterate through domain values of the selected variable
        for val in self.order_domain_values(var, assignment):

            # Track number of domain values tested to find solution
            WORDS_TESTED += 1

            # Create new assignment by adding selected variable's value
            new_assignment = deepcopy(assignment)
            new_assignment[var] = val

            # If new assignment is consistent, recursively call backtrack
            if self.consistent(new_assignment):
                result = self.backtrack(new_assignment)
                if result:
                    return result

        # Otherwise, no solution found so return None
        return None

    def backtrack_ac3(self, assignment):
        """
        Using interleaved Backtracking and Arc Consistency Search, take as input
        a partial assignment for the crossword and return a complete assignment
        ...
        # If no consistent assignment found, apply arc consistency
        self.ac3()
        return self.backtrack(dict())
